					...
					<form id="returnsForm" action="returns.jhtml" method="post">
						...
						<div style="height: 380px; overflow-x: hidden; overflow-y: auto;">
							<table class="input" style="margin-bottom: 30px;">
								...
								<tr>
									...
									<td>
										[#if order.hasShippingProduct]
											...
											<\/select>
										[/#if]
										[#if order.hasSmsProduct]
											${message("Order.smsShipping")}
										[/#if]
										[#if  order.hasIdCardProduct]
											${message("Order.idcardShipping")}
										[/#if]
									<\/td>
									<th>
										[#if order.hasShippingProduct]
								...
								<tr>
									<th>
										abc${message("Returns.trackingNo")}:
									<\/th>
									<td>
										<input type="text" name="trackingNo" class="text" maxlength="200" \/>
									<\/td>
	...
	<table class="input tabContent">
		<tr>
			...
			<td>
				...
				[#else]
				<input type="button" id="shippingButton" class="button" value="${message("admin.order.smsIdCard")}" disabled="disabled" />
				[/#if]
