...
$().ready(function() {
	...
	[#list order.orderItems as orderItem]
		[#if orderItem.product.productType==2]
			[#assign productType=2] 
		[/#if]
		
		[#if orderItem.product.productType==0]
			...
			$.ajax({
				...
				success: function(data) {
					if (data.message.type == "success") {
						...
						$("#receiver li").removeClass("selected");
						$receiver.append('<li class="selected" receiverId="' + data.receiver.id + '"><div><strong>' + data.receiver.consignee + '<\/strong> ${message("shop.order.receive")}<\/div><div [#if productType==0]class="hidden"[/#if]><span>' + data.receiver.areaName + data.receiver.address + '<\/span><\/div><div><strong>${message("shop.member.phone")}: <\/strong>' + data.receiver.phone + '<\/div><\/li>');
						$dialogOverlay.hide();
						$newReceiver.hide();
						[#if setting.isInvoiceEnabled]
							if ($.trim($invoiceTitle.val()) == "") {
		...
		<div class="span24">
			...
			<div class="info">
				<form id="receiverForm" action="save_receiver.jhtml" method="post">
					<div id="receiver" class="receiver clearfix">
						...
						<ul>
							[#list member.receivers as receiver]
								[#if receiver_index == 0]
									[#assign defaultReceiver = receiver /] 
									[#assign class="selected" /]
								[/#if]
								[#if receiver_index > 3 || (productType==0  && receiver.area.id!=10000) || (productType==2 && receiver.area.id==10000)]
									[#assign class="hidden" /]
								[#else]
									[#assign class="" /]
								[/#if]
								<li class ="${class}" receiverId="${receiver.id}">
									
									<div>
										<strong>${receiver.consignee}</strong> ${message("shop.order.receive")}
									</div>
									<div [#if productType==0]class="hidden"[/#if]>
										<span>${receiver.areaName}${receiver.address}</span>
									</div>
									<div>
										<strong>${message("shop.member.phone")}: </strong>${receiver.phone}
									</div>
								</li>
							[/#list]
						</ul>
						<p>
							[#if member.receivers?size > 8]
								<a href="javascript:;" id="otherReceiverButton" class="button">${message("shop.order.otherReceiver")}</a>
							[/#if]
							<a href="javascript:;" id="newReceiverButton" class="button"[#if member.receivers?size > 8] style="display: none;"[/#if]>${message("shop.order.newReceiver")}</a>
						</p>
					</div>
					<table id="newReceiver" class="newReceiver[#if member.receivers?has_content] hidden[/#if]">
						<tr>
							...
							<td>
								<input type="text" id="consignee" name="consignee" class="text" maxlength="200" value="${member.username}"/>
							</td>
						</tr>
						<tr [#if productType==0]class="hidden"[/#if]>
							<th>
								<span class="requiredField">*</span>${message("shop.order.area")}:
							</th>
							<td>
								<span class="fieldSet">
									<input type="hidden" id="areaId" name="areaId" [#if productType==0]value="10000"[/#if]/>
								</span>
							</td>
						</tr>
						<tr [#if productType==0]class="hidden"[/#if]>
							...
							<td>
								<input type="text" id="address" name="address" class="text" maxlength="200" [#if productType==0]value="default"[/#if]/>
							</td>
						</tr>
						<tr [#if productType==0]class="hidden"[/#if]>
							<th>
							...
							<td>
								<input type="text" id="zipCode" name="zipCode" class="text" maxlength="200" [#if productType==0]value="111111"[/#if]/>
							</td>
						</tr>
						<tr>
							<th>
				...
				<form id="orderForm" action="create.jhtml" method="post">
					...
					<dl id="shippingMethod"[#if productType==2] class="select"[#else] class="hidden"[/#if]>
						<dt>${message("shop.order.shippingMethod")}</dt>
						[#list shippingMethods as shippingMethod]
							<dd [#if productType==2 && shippingMethod.id==1] class="hidden"[/#if]>
								<label for="shippingMethod_${shippingMethod.id}">
									<input type="radio" id="shippingMethod_${shippingMethod.id}" name="shippingMethodId" [#if productType==2]value="${shippingMethod.id}"[#else]value="${shippingMethod.id}" checked[/#if] />
									<span>
