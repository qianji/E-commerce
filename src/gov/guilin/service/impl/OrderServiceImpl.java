...
import gov.guilin.dao.CouponCodeDao;
import gov.guilin.dao.DepositDao;
import gov.guilin.dao.MemberDao;
import gov.guilin.dao.MemberRankDao;
...
import gov.guilin.dao.ProductDao;
...
import gov.guilin.entity.GiftItem;
...
import gov.guilin.entity.ReturnsItem;
import gov.guilin.entity.Shipping;
import gov.guilin.entity.ShippingItem;
...
import gov.guilin.entity.OrderLog.Type;
...
public class OrderServiceImpl extends BaseServiceImpl<Order, Long> implements OrderService {
	...
	public Page<Order> findPage(OrderStatus orderStatus, PaymentStatus paymentStatus, ShippingStatus shippingStatus, Boolean hasExpired, Pageable pageable ,Set<Supplier> suppliers) {
		return orderDao.findPage(orderStatus, paymentStatus, shippingStatus, hasExpired, pageable,suppliers);
	}
	 ...
	 * @return
	 */
	public boolean canSubmitOrder(List<Product> products) {
		try {
			int productType = products.get(0).getProductType();
			long supplierId = products.get(0).getSupplier().getId();
			for (Product product : products) {
				if ((product.getProductType() != productType) || (supplierId != product.getSupplier().getId())) {
					return false;
				}
			}
			return true;
