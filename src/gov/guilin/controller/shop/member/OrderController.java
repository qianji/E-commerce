...
import gov.guilin.service.CouponCodeService;
...
public class OrderController extends BaseController {
	 ...
	 * 是否可提交检测
	 */
	@RequestMapping(value = "/cansubmit", method = RequestMethod.GET)
	public @ResponseBody
	Map<String, String> canSubmitOrder() {
		...
		if (cart == null || cart.isEmpty()) {
			data.put("code", "500");
			data.put("msg", Message.warn("shop.order.cartNotEmpty").toString());
			return data;
		}

		// 表单是否可提交
		List<Product> products = new ArrayList<Product>();
		Set<CartItem> cardItems = cart.getCartItems();
		for (CartItem cartItem : cardItems) {
			products.add(cartItem.getProduct());
		}
		if (!orderService.canSubmitOrder(products)) {
			data.put("code", "500");
			data.put("msg", Message.warn("shop.order.orderCannotCommit").toString());
			return data;
		}
		data.put("code", "200");
		return data;
	...
	Message create(String cartToken, Long receiverId, Long paymentMethodId, Long shippingMethodId, String code, @RequestParam(defaultValue = "false") Boolean isInvoice, String invoiceTitle, @RequestParam(defaultValue = "false") Boolean useBalance, String memo) {
		...
		if(!orderService.canSubmitOrder(products)){
			return Message.warn("shop.order.orderCannotCommit");
		}
